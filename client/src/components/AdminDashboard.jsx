...
export default function AdminDashboard() {
  ...
  const openTriageModal = (complaint) => {
    ...
    const catId =
      complaint.matchedCategory?.id ||
      complaint.matchedCategory?._id ||
      (typeof complaint.matchedCategoryId === 'object'
        ? complaint.matchedCategoryId?.id || complaint.matchedCategoryId?._id
        : complaint.matchedCategoryId);
    setTriageCategoryId(catId ? String(catId) : '');
  ...
  const handleSaveTriage = async () => {
    ...
    try {
      const updatePayload = {
        ...
        matchedCategoryId: triageCategoryId ? String(triageCategoryId) : null,
  ...
  const handleAutoRoute = async () => {
    ...
    try {
      ...
      const catId =
        res.complaint?.matchedCategory?.id ||
        res.complaint?.matchedCategory?._id ||
        (typeof res.complaint?.matchedCategoryId === 'object'
          ? res.complaint?.matchedCategoryId?.id || res.complaint?.matchedCategoryId?._id
          : res.complaint?.matchedCategoryId);
      setTriageCategoryId(catId ? String(catId) : '');
